...
- Module 5 realignment design decisions, confirmed with client 2026-09-22 ("pwede sa admin muna" — route through university_admin, refine later if the Manual said otherwise):
  ...
  - >100%/new-expense-item tier (Board of Regents, no BOR role exists) is reference-only: `university_admin` records a `bor_resolution_number` once externally approved by BOR; there's no in-system BOR approval workflow. Client confirmed this matches their "Approved BOR" pinned feature.
